          ...
          child: Consumer<DriverProposalsViewModel>(
            builder: (context, viewModel, _) {
              return CustomScrollView(
                slivers: [
                  SliverToBoxAdapter(
                    child: _HeaderSection(email: widget.session.email),
                  ),
                  if (viewModel.isLoading && viewModel.proposals.isEmpty)
                    const SliverFillRemaining(
                      child: Center(child: CircularProgressIndicator()),
                    )
                  else if (viewModel.errorMessage != null && viewModel.proposals.isEmpty)
                    SliverFillRemaining(
                      child: Padding(
...
class _ProposalCard extends StatelessWidget {
  ...
  Color _statusColor(String status) {
    ...
    if (normalized.contains('aceit')) return const Color(0xFF10B981);
    if (normalized.contains('cancel')) return const Color(0xFFEF4444);
    if (normalized.contains('and')) return const Color(0xFFF59E0B);
